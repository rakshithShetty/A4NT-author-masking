...
class CharLstm(nn.Module):
    def __init__(self, params):
        ...
        self.rec_size = params.get('hidden_size',-1)
        self.en_residual = params.get('residual_conn',1)

        # Initialize the model layers
        # Embedding layer
        self.encoder = nn.Embedding(self.output_size, self.emb_size, padding_idx = -1)
        ...
        self.decoder_b = nn.Parameter(torch.zeros([self.num_output_layers, self.hidden_size,
            self.output_size]), True)
    ...
    def init_weights(self):
        ...
        if self.en_residual:
          for i in xrange(self.num_rec_layers):
            self.rec_layers[i].b_ih_l0.data.index_fill_(0, tensor.arange(h_sz +1, h_sz*2).long(), 1.)
            self.rec_layers[i].b_hh_l0.data.index_fill_(0, tensor.arange(h_sz +1, h_sz*2).long(), 1.)
        else:
          for i in xrange(self.num_rec_layers):
            getattr(self.rec_layers,'b_ih_l'+str(i)).data.index_fill_(0, tensor.arange(h_sz +1, h_sz*2).long(), 1.)
            getattr(self.rec_layers,'b_hh_l'+str(i)).data.index_fill_(0, tensor.arange(h_sz +1, h_sz*2).long(), 1.)

    def init_hidden(self, bsz):
        # Weight initializations for various parts.
        weight = next(self.parameters()).data
        return (Variable(weight.new(self.num_rec_layers, bsz, self.rec_size).zero_()),
                    Variable(weight.new(self.num_rec_layers, bsz, self.rec_size).zero_()))


    def forward(self, x, h_prev, target_head):
        ...
        if self.en_residual:
            ...
            hid_all = []
            for i in xrange(self.num_rec_layers):
                out, hid = self.rec_layers(p_out, (h_prev[0][i:i+1], h_prev[1][i:i+1]))
                if i > 0:
            ...
            rnn_out, hidden = self.rec_layers(emb, h_prev)

        rnn_out = self.dec_drop(rnn_out)

        # implement the multi-headed RNN.
        W = self.decoder_W[target_head]
        # reshape and expand b to size (batch*n_steps*vocab_size)
        b = self.decoder_b[target_head].view(b_sz, -1, self.output_size)
        b = b.expand(b_sz, x.size(0), self.output_size)
        # output is size seq * batch_size * vocab
        dec_out = torch.baddbmm(m, rnn_out.transpose(0,1), w).transpose(0,1)
